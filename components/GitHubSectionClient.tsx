...
export function GitHubSectionClient({
  user,
  featuredRepos,
  githubYears,
}: GitHubSectionClientProps) {
  const { locale, t } = useI18n();
  const section = t.sections.github;
  ...
  return (
    <Section
      ...
      tag={section.tag}
      title={section.title}
      description={section.description.replace("@JefiiRamos", `@${GITHUB_USERNAME}`)}
    >
      <div className="github-layout">
          ...
          <div className="github-profile__header">
            ...
            <div>
              ...
              <h3 className="github-profile__name">
                {user?.name?.trim() || t.profile.name}
              </h3>
              <p className="github-profile__since">
                {section.since}{" "}
                {user ? new Date(user.created_at).getFullYear() : "2023"} ·{" "}
                {githubYears}+ {section.years}
              </p>
            </div>
          </div>
          ...
          <div className="github-profile__stats">
            <div className="github-mini-stat">
              <span className="github-mini-stat__value">
                {user?.public_repos ?? featuredRepos.length}
              </span>
              <span className="github-mini-stat__label">{section.repositories}</span>
            </div>
            <div className="github-mini-stat">
              <span className="github-mini-stat__value">{user?.followers ?? 0}</span>
              <span className="github-mini-stat__label">{section.followers}</span>
            </div>
            <div className="github-mini-stat">
              <span className="github-mini-stat__value">{user?.following ?? 0}</span>
              <span className="github-mini-stat__label">{section.following}</span>
            </div>
          </div>
        ...
        <article className="panel github-readme">
          <p className="github-readme__tag">{section.profileRead}</p>
          <ul className="github-readme__list">
            {section.profilePoints.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          <div className="github-readme__goals">
            <p className="github-readme__goals-title">{section.goalsTitle}</p>
            <ul>
      ...
      {featuredRepos.length > 0 ? (
        <div className="github-repos">
          <p className="github-repos__label">{section.featuredRepos}</p>
            ...
            {featuredRepos.map((repo) => (
              <article key={repo.id} className="panel repo-card-v2">
                ...
                <p className="repo-card-v2__desc">
                  {getRepoDescription(repo, locale)}
                </p>
